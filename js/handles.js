  ...
  */
  $("#createReviewPage").click(function() {
    ...
    $(".ticketOrder").each(function () {
      ...
      var priceClass = uid.substring(uid.length-3, uid.length-4);
  ...
  */
  ...
  $("#addticketbutton").click(function() {
    ...
    $.getJSON( "ticket_reservation.php?cart", function(ticket) {
      tickets++;
      $(this).before('<input type="text" class="wide ticketOrder" id="name-ticket'+ tickets +'" name="tickets[owner][]" placeholder="z.B. Max Mustermann"><input type="hidden" name="tickets[uids][]" value="'+ticket.uid+'"><button type="button" class="removeTicketButton">-</button><br>');
    });
  });
